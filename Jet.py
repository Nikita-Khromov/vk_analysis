...
for i in range(0, 100):
    for j in range(0, len(comments[i].get('items'))):
        ...
        elif (comments[i].get('items')[j].get('likes') != None) and (likes_count != 0) and (user_id > 0):
            usersByLikes[user_id] = likes_count 

for user in usersByLikes.keys():
    likesData[0].append(user)
    likesData[1].append(vk.users.get(user_ids = user)[0].get('first_name') + " " + vk.users.get(user_id = user)[0].get('last_name'))
